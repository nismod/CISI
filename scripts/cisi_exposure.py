...
def base_calculations(infrastructure_systems, fetched_data_dict, grid_data):
    ...
    print(
        "The counts, lengths and areas of each asset will now be calculated for each grid cell"
    )

    # loop through infrastructure systems
    for ci_system in infrastructure_systems:
        print(
            "The loop starts for subsystem {} with assets for the following groups {}".format(
                ci_system, infrastructure_systems[ci_system]
            )
        )
        for value in infrastructure_systems[ci_system]:
            if fetched_data_dict[value].empty == False:
                ...
                if len(list_datatypes) == 1:
                    ...
                    geometry_type = pygeos.geometry.get_type_id(
                        fetched_data_dict[value].iloc[0]["geometry"]
                    )  # get geometry id
                    if geometry_type == 0:  # point
                        # print("{}_{}.gpkg".format(area, value), "has geometrytype", geometry_file)
                        cisi_exposure[ci_system] = cisi.count_per_grid_pygeos(
                            fetched_data_dict[value], cisi_exposure[ci_system]
                        )
                    elif geometry_type == 1:  # "linestring" in geometry_type:
                        # print("{}_{}.gpkg".format(area, value), "has geometrytype", geometry_file)
                        cisi_exposure[ci_system] = cisi.length_km_per_grid_pygeos(
                            fetched_data_dict[value], cisi_exposure[ci_system]
                        )
                    elif geometry_type == 3:  # "polygon" in geometry_type:
                        # print("{}_{}.gpkg".format(area, value), "has geometrytype", geometry_file)
                        fetched_data_dict[value]["geometry"] = pygeos.buffer(
                            fetched_data_dict[value].geometry, 0
                        )
                        cisi_exposure[ci_system] = cisi.area_km2_per_grid_pygeos(
                            fetched_data_dict[value], cisi_exposure[ci_system]
                        )
                        ...
                        print(
                            "WARNING: fetched data '{}' contains an unexpected geometry type {}".format(
                                value, geometry_type
                            )
                        )
                elif len(list_datatypes) > 1:
                    cisi_exposure[ci_system] = base_calculations_multipledatatypes(
                        list_datatypes,
                        fetched_data_dict[value],
                ...
                print(
                    "WARNING: the following group does not exist: {}. Empty df is returned".format(
                        value
                    )
                )
        ...
        for col in cisi_exposure[ci_system].columns:
            if "_count" in col or "_km" in col or "_km2" in col:
                cisi_exposure[ci_system][col] = cisi_exposure[ci_system][col].astype(
                    float
                )
            # else:
...
def base_calculations_multipledatatypes(list_datatypes, infra_dataset, df_store):
    ...
    infra_dataset["datatype"] = pygeos.geometry.get_type_id(
        infra_dataset["geometry"]
    )  # add column specifying datatype
    infra_dataset["datatype"].replace(
        {1: 5, 3: 6}, inplace=True
    )  # do not differentiate between linestrings-multilinestrings, polygons-multipolygons

    for item in list_datatypes:
        if item == 0:  # point data
            df_store_point = copy.deepcopy(df_store)
            point_df = (
                infra_dataset[infra_dataset["datatype"] == 0]
                .drop(["datatype"], axis=1)
                .reset_index(drop=True)
            )
            grid_point_df = cisi.count_per_grid_pygeos(point_df, df_store_point)
            df_final = pd.concat(
                [df_final, grid_point_df.drop(["geometry", "grid_number"], axis=1)],
                axis=1,
            )
        elif item == 5:  # line data
            df_store_line = copy.deepcopy(df_store)
            line_df = (
                infra_dataset[infra_dataset["datatype"] == 5]
                .drop(["datatype"], axis=1)
                .reset_index(drop=True)
            )
            grid_line_df = cisi.length_km_per_grid_pygeos(line_df, df_store_line)
            df_final = pd.concat(
                [df_final, grid_line_df.drop(["geometry", "grid_number"], axis=1)],
                axis=1,
            )
        elif item == 6:  # polygon data
            df_store_polygon = copy.deepcopy(df_store)
            polygon_df = (
                infra_dataset[infra_dataset["datatype"] == 6]
                .drop(["datatype"], axis=1)
                .reset_index(drop=True)
            )
            grid_polygon_df = cisi.area_km2_per_grid_pygeos(
                polygon_df, df_store_polygon
            )
            df_final = pd.concat(
                [df_final, grid_polygon_df.drop(["geometry", "grid_number"], axis=1)],
                axis=1,
            )
    ...
    for ci_system in infrastructure_systems:
        ...
        for value in infrastructure_systems[ci_system]:
            if fetched_data_dict[value].empty == False:
                ...
                if geometry_type == 0:  # point
                    # print("{}_{}.gpkg".format(area, value), "has geometrytype", geometry_file)
                    cisi.count_per_grid_pygeos(
                        fetched_data_dict[value], cisi_exposure[ci_system]
                    )
                elif geometry_type == 1:  # "linestring" in geometry_type:
                    # print("{}_{}.gpkg".format(area, value), "has geometrytype", geometry_file)
                    cisi.length_km_per_grid_pygeos(
                        fetched_data_dict[value], cisi_exposure[ci_system]
                    )
                elif geometry_type == 3:  # "polygon" in geometry_type:
                    # print("{}_{}.gpkg".format(area, value), "has geometrytype", geometry_file)
...
def unique_datatypes(gdf):
    ...
    for index, item in enumerate(list_datatypes):
        if item == 1:  # model does not differentiate between strings and multistrings
            ...
            item == 3
        ):  # model does not differentiate between polygon and multipolygons
...
def cisi_overall_max_single_no_normalization(
    weight_assets,
    weight_groups,
    weight_subsystems,
    infrastructure_systems,
        ...
        for value in infrastructure_systems[ci_system]:
            ...
            for asset in weight_assets[ci_system][value]:
                if (
                    ...
                    temp[temp == 0] = np.nan
                    if (
                        temp.isnull().all() == False
                    ):  # whole column does not contain nans
                        cisi_exposure[ci_system]["Index_{}".format(asset)] = (
                            cisi_exposure[ci_system][asset] / temp.mean()
                        )  # add column km/max km, calculate asset index
                    else:
                        cisi_exposure[ci_system]["Index_{}".format(asset)] = 0.0
                    cisi_exposure[ci_system]["Index_{}".format(value)] += cisi_exposure[
...
def cisi_max_per_area(
    ...
    for ci_system in infrastructure_systems:
        ...
        ] = 0  # create column in cisi_exposure to calculate the index of the subsystems
        for value in weight_groups[ci_system]:
            if value in infrastructure_systems[ci_system]:
                cisi_exposure[ci_system]["Index_{}".format(ci_system)] += cisi_exposure[
                    ci_system
    ...
    for ci_system in infrastructure_systems:
        for value in infrastructure_systems[ci_system]:
            ...
            for asset in weight_assets[ci_system][value]:
                if (
                    asset in cisi_exposure[ci_system].columns
                ):  # check whether asset in dictioniary exists in cisi exposure dataframe
                    cisi_exposure[ci_system]["Index_{}".format(asset)] = (
                        cisi_exposure[ci_system][asset]
                        / overall_statistics_tuple[method_list[0]][degree][ci_system][
                            value
                        ][asset]
                    )  # add column km/max km, calculate asset index
                    cisi_exposure[ci_system]["Index_{}".format(value)] += cisi_exposure[
                    ...
                    print(
                        "\033[1mNOTIFICATION: The following asset is non-existent in cisi_exposure geodataframes: {}, {} \033[0m \ncheck if extracting codes have been written correctly, otherwise, consider to exclude asset from index".format(
                            area, asset
                        )
                    )
...
def overall_statistics_per_asset_dict(
    areas, weight_assets, degrees, infrastructure_systems, make_histograms=False
):  # degrees, weight_assets, infrastructure_systems, output_infra_base_gpkg_path):
    ...
    asset_dict_temp = copy.deepcopy(
        weight_assets
    )  # use deepcopying for nested dictionaries https://stackoverflow.com/questions/39474959/nested-dictionaries-copy-or-deepcopy
    ...
    overall_max_dict = {
        degree: copy.deepcopy(asset_dict_temp) for degree in degrees
    }  # prepare max_per_asset dictionary
    overall_mean_dict = {
        degree: copy.deepcopy(asset_dict_temp) for degree in degrees
    }  # prepare mean_per_asset dictionary
    overall_min_dict = {
        degree: copy.deepcopy(asset_dict_temp) for degree in degrees
    }  # prepare min_per_asset dictionary
            ...
            for continent in areas:
                for area in areas[continent]:
                    if (
                        os.path.isfile(
                            os.path.join(
                                output_infra_base_gpkg_path,
                                continent,
                                area,
                                "gridsize_{}".format(degree),
                        ...
                        infra_base_data = (
                            ...
                            ["index"], axis=1
                        )  # open grid data and delete index column
                        temp_df_infra_base_data = temp_df_infra_base_data.append(
                            infra_base_data, ignore_index=True, sort=False
                        )  # put data of subsystem per area in one dataframe
                        # print(temp_df_infra_base_data)
                    else:
                        print(
                            "WARNING: the following basefile does not exist: {}".format(
                                os.path.join(
                                    output_infra_base_gpkg_path,
                                    continent,
                                    area,
            ...
            for value in infrastructure_systems[ci_system]:
                for asset in overall_mean_dict[degree][ci_system][value]:
                    ...
                    if asset in temp_df_infra_base_data:
                        overall_max_dict[degree][ci_system][value][asset] = (
                            temp_df_infra_base_data[asset].max()
                        )

                        temp_df_infra_base_data[asset][
                            temp_df_infra_base_data[asset] == 0
                        ] = np.nan  # prior to calculating mean, replace 0 with nans
                        overall_mean_dict[degree][ci_system][value][asset] = (
                            temp_df_infra_base_data[asset].mean()
                        )
                        overall_min_dict[degree][ci_system][value][asset] = (
                            temp_df_infra_base_data[asset].min()
                        )

                        if make_histograms == True:
                            make_histogram_automatic(
                                ci_system,
                                asset,
                                degree,
                                temp_df_infra_base_data,
                                temp_df_infra_base_data[asset].max(),
                                temp_df_infra_base_data[asset].mean(),
                                temp_df_infra_base_data[asset].min(),
    ...
    for ci_system in weight_subsystems:  # loop over each CI subsystem
        ...
        if ci_system in infrastructure_systems:
            ...
            exposure_index["CISI_exposure_unnormalized"] += exposure_index[
                "Subscore_{}".format(ci_system)
            ]  # column for total score
...
def make_histogram_automatic(
    ...
    if "_count" in asset:
        ax.set_title(
            "Histogram of asset: {}, {}".format(asset.replace("_count", ""), ci_system),
            size=12,
        )
        ax.set_xlabel("Number of assets per grid", size=10)
    elif "_km2" in asset:
        ax.set_title(
            "Histogram of asset: {}, {}".format(asset.replace("_km2", ""), ci_system),
            size=12,
        )
        ax.set_xlabel("Area of asset per grid ($\mathregular{km^2}$)", size=10)
    elif "_km" in asset:
        ax.set_title(
            "Histogram of asset: {}, {}".format(asset.replace("_km", ""), ci_system),
            size=12,
        )
    ...
    ax.xaxis.set_minor_locator(
        AutoMinorLocator()
    )  # see https://matplotlib.org/api/ticker_api.html?highlight=fixedlocator#module-matplotlib.ticker
    ...
    plt.axvline(
        ...
        color="white",
        linewidth=0,
        label="minimum: {:5.1f}".format(min_value),
    )
    plt.axvline(
        mean_value,
        color="slategrey",
        linestyle="dashed",
        linewidth=2,
        label="mean: {:12.1f}".format(mean_value),
    )
    plt.axvline(
        max_value,
        color="white",
        linewidth=0,
        label="maximum: {:5.1f}".format(max_value),
    )
    ...
    plt.savefig(
        os.path.join(
            output_histogram_path,
            "histogram_gridsize_{}_{}_{}.png".format(degree, ci_system, asset),
        ),
        bbox_inches="tight",
    )
    plt.close(fig)
...
def make_plots_automatic(cisi_exposure_output, exposure_index, goal_area, output_path):
    ...
    for ci_system in cisi_exposure_output:
        cisi_exposure_output[ci_system]["geometry"] = cisi_exposure_output[
            ci_system
        ].geometry.apply(
            lambda x: loads(pygeos.to_wkb(x))
        )  # transform geometry back to shapely geometry
        cisi_exposure_output[ci_system] = gpd.GeoDataFrame(
            cisi_exposure_output[ci_system], crs="EPSG:4326", geometry="geometry"
        )
        ...
        xlim = [
            cisi_exposure_output[ci_system]["geometry"].total_bounds[0],
            cisi_exposure_output[ci_system]["geometry"].total_bounds[2],
        ]
        ylim = [
            cisi_exposure_output[ci_system]["geometry"].total_bounds[1],
            cisi_exposure_output[ci_system]["geometry"].total_bounds[3],
        ]
        ...
        for col in cisi_exposure_output[ci_system].columns:
            if "Index" in col:
                ...
                if "Index_{}".format(ci_system) == col:
                    ...
                    plt.title(
                        "{}: {}, {} (max.={:.3f})".format(
                            ...
                            "Index_{}".format(ci_system),
                            goal_area,
                            cisi_exposure_output[ci_system][col].max(),
                        ),
                        x=-10,
                    )
                    cisi_exposure_output[ci_system].plot(
                        column=col,
                        cmap="gist_heat_r",
                        legend=True,
                    ...
                    plt.savefig(
                        os.path.join(
                            output_path, "Subsystem_Index_{}.png".format(ci_system)
                        ),
                        bbox_inches="tight",
                    )
                    plt.close(fig)
                ...
                # plot assets and groups
                    ...
                    plt.savefig(
                        os.path.join(output_path, "{}_{}.png".format(ci_system, col)),
                        bbox_inches="tight",
                    )
                    plt.close(fig)

    # plot of final_exposure
    exposure_index["geometry"] = exposure_index.geometry.apply(
        lambda x: loads(pygeos.to_wkb(x))
    )  # transform geometry back to shapely geometry
    exposure_index = gpd.GeoDataFrame(
        exposure_index, crs="EPSG:4326", geometry="geometry"
    )
    col = "CISI"
    fig, ax = plt.subplots(figsize=(15, 7))
...
def transform_to_gpd(df1):
    ...
    temp_df = df1.copy()
    temp_df["geometry"] = temp_df.geometry.apply(
        lambda x: loads(pygeos.to_wkb(x))
    )  # transform geometry back to shapely geometry
...
def get_documentation(
    ...
    weight_subsystems,
    overall_statistics_tuple,
    output_documentation_path,
    test_number,
):
    with open(
        os.path.join(
            output_documentation_path, "documentation_{}.txt".format(test_number)
        ),
        "w+",
    ) as f:
        f.write(
            "This file contains documentation on the exposure index: \n\
        ...
        {}".format(
                weight_assets, weight_groups, weight_subsystems
            )
        )

        for degree in degrees:
            f.write(
                "\n\n\nFor index calculations when the gridsize is {} degrees".format(
                    degree
                )
            )
            ...
            {}".format(
                    overall_statistics_tuple[0][degree]
                )
            )
            ...
            {}".format(
                    overall_statistics_tuple[1][degree]
                )
            )
            ...
            {}".format(
                    overall_statistics_tuple[2][degree]
                )
            )
