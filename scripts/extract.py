...
def merge_energy_datatypes(osm_path):
    ...
    if "asset" in df_line.columns:
        df_line["asset"] = list(
            map(lambda x: x.lower(), df_line["asset"])
        ...
        df_line["asset"] = df_line.asset.apply(
...
def power_polygon(osm_path):
    ...
    df = df.loc[
        (df.other_tags.str.contains("power"))
    ]  # keep rows containing power data
    df = df.reset_index(drop=True).rename(columns={"other_tags": "asset"})

    df["asset"].loc[df["asset"].str.contains('"power"=>"substation"', case=False)] = (
        "substation"  # specify row
    )
    df["asset"].loc[df["asset"].str.contains('"power"=>"plant"', case=False)] = (
        "plant"  # specify row
    )
...
def power_point(osm_path):
    ...
    df["asset"].loc[df["asset"].str.contains('"power"=>"tower"', case=False)] = (
        "power_tower"  # specify row
    )
    df["asset"].loc[df["asset"].str.contains('"power"=>"pole"', case=False)] = (
        "power_pole"  # specify row
    )
...
def roads_all(osm_path):
    ...
    """
    return (retrieve(osm_path, "lines", ["highway"])).rename(
        columns={"highway": "asset"}
    )
...
def airports(osm_path):
    ...
    return (
        retrieve(
            osm_path, "multipolygons", ["aeroway"], **{"aeroway": ["='aerodrome'"]}
        )
    ).rename(columns={"aeroway": "asset"})
...
def railway_service(osm_path):
    ...
    """
    return retrieve(
        osm_path, "lines", ["railway", "service"], **{"service": [" IS NOT NULL"]}
    )
...
def railway_all(osm_path):
    ...
    """
    return (retrieve(osm_path, "lines", ["railway"])).rename(
        columns={"railway": "asset"}
    )
...
def railway_stops(osm_path):
    ...
    return retrieve(
        ...
        "points",
        ["railway"],
        **{"railway": ["='halt' or ", "='subway_entrance' or ", "='tram_stop'"]}
    )
...
def railway_areas(osm_path):
    ...
    return retrieve(
        ...
        ["railway", "landuse"],
        **{
            "railway": ["='platform' or ", "='station' or ", "='tram_stop'"],
            "landuse": ["='railway'"],
        }
    )
...
def ports(osm_path):
    ...
    return (
        retrieve(
            ...
            "multipolygons",
            ["landuse"],
            **{"landuse": ["='industrial' or ", "='port' or ", "='harbour'"]}
        )
    ).rename(columns={"landuse": "asset"})
...
def water_supply(osm_path):
    ...
    i = retrieve(
        ...
        **{
            "man_made": [
                "='water_tower' or ",
                "='water_well' or ",
                "='reservoir_covered' or ",
                "='water_works'",
            ]
        }
    )
    j = retrieve(
        osm_path, "multipolygons", ["landuse"], **{"landuse": ["='reservoir'"]}
    )
    ...
    combined_df = pandas.concat(
        [i, j], ignore_index=True, sort=False
    )  # append objects while ignoring that they may have overlapping index
...
def telecom_mast(osm_path):
    ...
    """
    df = retrieve(
        osm_path, "points", ["man_made", "other_tags"], **{"man_made": ["='mast'"]}
    ).rename(columns={"man_made": "asset"})
...
def telecom_towers(osm_path):
    ...
    combined_df = pandas.concat(
        [s, b], ignore_index=True, sort=False
    )  # append objects while ignoring that they may have overlapping index
    if combined_df.empty == True:
        return combined_df
    else:
...
def telecom_towers_small(osm_path):
    ...
    """
    df = retrieve(
        osm_path, "points", ["man_made", "other_tags"], **{"man_made": ["='tower'"]}
    ).rename(columns={"man_made": "asset"})
...
def telecom_towers_small2(osm_path):
    ...
    telecom_towers = geopandas.GeoDataFrame(
        columns=["osm_id", "man_made", "other_tags", "geometry"]
    )
    ...
    for row in range(len(df_all.index)):
        if "healthcare" in df_all["asset"][row]:  # check if healthcare key is present
            ...
            if '"healthcare"=>"doctor"' in df_filtered["asset"][row]:
                df_filtered["asset"][
                    row
                ] = "doctors"  # to be consistent with asset list
...
def social_healthcare(osm_path):
    ...
    Returns:
        *GeoDataFrame* : a geopandas GeoDataFrame with specified unique healthcare polygons.
    """
    df_all = retrieve(osm_path, "multipolygons", ["other_tags", "amenity"]).rename(
        columns={"other_tags": "asset"}
    )
    ...
    asset_list = [
        ...
        "dentist",
        "pharmacy",
    ]  # note that this list of assets should be similar to assets extracted in def social_amenity
    for asset in asset_list:
        index_delete = df_all[(df_all["amenity"] == asset)].index
        df_all.drop(index_delete, inplace=True)
    df_all = df_all.drop(["amenity"], axis=1).reset_index(
        drop=True
    )  # drop amenity column, reset index
...
def social_infrastructure_point(osm_path):
    ...
    """
    df_all = retrieve(osm_path, "points", ["other_tags"]).rename(
        columns={"other_tags": "asset"}
    )
    ...
    for row in range(len(df_all.index)):
        if "amenity" in df_all["asset"][row]:
            if (
                not "healthcare" in df_all["asset"][row]
            ):  # check if healthcare key is present
...
def social_infrastructure_combined(osm_path):
    ...
    df_polygon_filtered = compare_polygon_to_point(
        df_point, df_polygon
    )  # remove duplicates polygon and point data
    df_polygon_filtered["geometry"] = pygeos.centroid(
        df_polygon_filtered.geometry
    )  # transform to pointdata
