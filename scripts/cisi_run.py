...
def run_all(
    areas, goal_area="Global", local_path=os.path.join("/scistor", "ivm", "snn490")
):
...
def set_variables():
    ...
    weight_groups = {
        "energy": {"power": 1},
        "transportation": {"roads": 1 / 3, "airports": 1 / 3, "railways": 1 / 3},
...
def set_paths(
    local_path="C:/Data/CISI",
    extract_data=False,
    base_calculation=False,
    cisi_calculation=False,
):
    ...
    osm_data_path = os.path.abspath(
        os.path.join(
            "/scistor", "ivm", "data_catalogue", "open_street_map", "country_osm"
        )
    )  # path to map with pbf files from OSM at cluster
    ...
    grid_path = os.path.abspath(
        os.path.join(local_path, "Outputs", "Grid_data", grid_file)
    )  # grid data
    shapes_file = "global_countries_advanced.geofeather"
    country_shapes_path = os.path.abspath(
        os.path.join(
            local_path,
            "Datasets",
            "Administrative_boundaries",
            "global_countries_buffer",
            shapes_file,
        )
    )  # shapefiles with buffer around country

    # Set path for outputs
    base_path = os.path.abspath(
        os.path.join(local_path, "Outputs", "Exposure", "CISI_global")
    )  # this path will contain folders in which

    # path to save outputs - automatically made, not necessary to change output pathways
    fetched_infra_path = os.path.abspath(
        os.path.join(base_path, "Fetched_infrastructure")
    )  # path to map with fetched infra-gpkg's
    ...
    infra_base_path = os.path.abspath(
        os.path.join(base_path, "Infrastructure_base_025")
    )  # save interim calculations
    method_max_path = os.path.abspath(
        os.path.join(base_path, "index_025", "method_max")
    )  # save figures
    method_mean_path = os.path.abspath(
        os.path.join(base_path, "index_025", "method_mean")
    )  # save figures
...
def extract_infrastructure_per_area(
    area, groups_list, osm_data_path, fetched_infra_path, country_shapes_path
):
    ...
    fetched_data_dict = {
        group: pd.DataFrame() for group in groups_list
    }  # Create dictionary with asset groups as keys and df as value
    print("\033[1mTime to extract infrastructure data for area: {}\033[0m".format(area))
    for group in groups_list:
        print(
            "Infrastructure belonging to the group '{}' will now be extracted for {}".format(
                group, area
            )
        )
        data = "{}.osm.pbf".format(area)  # make directory to data
        if group == "power":
            fetched_data_area = extract.merge_energy_datatypes(
                os.path.join(osm_data_path, data)
            )  # extract required data
            if "asset" in fetched_data_area.columns:
                fetched_data_area["asset"] = list(
                    map(lambda x: x.lower(), fetched_data_area["asset"])
                )  # make sure that asset column is in lowercase charachters
        elif group == "roads":
            fetched_data_area = extract.roads_all(
                os.path.join(osm_data_path, data)
            )  # extract required data
            if "asset" in fetched_data_area.columns:
                fetched_data_area["asset"] = list(
                ...
                list_of_highway_assets_to_keep = [
                    "living_street",
                    "motorway",
                    "motorway_link",
                    "primary",
                ...
                fetched_data_area = fetched_data_area.loc[
                    fetched_data_area.asset.isin(list_of_highway_assets_to_keep)
                ].reset_index()
                # reclassify assets
                mapping_dict = {
                ...
                fetched_data_area["asset"] = fetched_data_area.asset.apply(
            ...
            if "asset" in fetched_data_area.columns:
                fetched_data_area["asset"] = list(
                    map(lambda x: x.lower(), fetched_data_area["asset"])
                )  # make sure that asset column is in lowercase characters
                # reclassify assets
                mapping_dict = {
                    "aerodrome": "airports",
                }
                ...
                fetched_data_area["geometry"] = pygeos.buffer(
                    fetched_data_area.geometry, 0
                )  # avoid intersection
        elif group == "railways":
            ...
            if "asset" in fetched_data_area.columns:
                ...
                list_of_railway_assets_to_keep = [
                    "rail",
                    "tram",
                    "subway",
                    "construction",
                ...
                fetched_data_area = fetched_data_area.loc[
                    fetched_data_area.asset.isin(list_of_railway_assets_to_keep)
                ].reset_index()
                # reclassify assets
                mapping_dict = {
                    "rail": "railway",
            ...
            if "asset" in fetched_data_area.columns:
                ...
                # reclassify assets
                mapping_dict = {"wastewater_plant": "wastewater_treatment_plant"}
                fetched_data_area["asset"] = fetched_data_area.asset.apply(
                    lambda x: mapping_dict[x]
                )  # reclassification
                fetched_data_area["geometry"] = pygeos.buffer(
            ...
            fetched_data_area = extract.social_infrastructure_combined(
                os.path.join(osm_data_path, data)
            )
            if "asset" in fetched_data_area.columns:
                fetched_data_area["asset"] = list(
            ...
            print(
                "WARNING: No extracting codes are written for the following area and group: {} {}".format(
                    area, group
                )
            )
        ...
        if country_shape.empty == False:  # if ISO_3digit in shape_countries
            ...
            fetched_data_area = cisi.clip_pygeos(
                fetched_data_area, country_shape.iloc[0], spat_tree
            )
        else:
            print(
                "ISO_3digit code not specified in file containing shapefiles of country boundaries. Floating data will not be removed for area '{}'".format(
                    area
                )
            )

        # Save data in df
        fetched_data_dict[group] = fetched_data_dict[group].append(
            fetched_data_area, ignore_index=True
        )
    ...
    if cisi.check_dfs_empty(fetched_data_dict) == False:  # df's contain data
        for group in groups_list:
            ...
            if fetched_data_dict[group].empty == False:
                print(
                    "Extraction of requested infrastructure is complete for group '{}' in area '{}'. This data will now be exported as geofeather...".format(
                        group, area
                    )
                )
                ...
                temp_df.to_file(
                    os.path.join(fetched_infra_path, "{}_{}.gpkg".format(area, group)),
                    layer=" ",
                    driver="GPKG",
                )
                ...
                to_geofeather(
                    fetched_data_dict[group],
                    os.path.join(
                        fetched_infra_path, "{}_{}.feather".format(area, group)
                    ),
                    crs="EPSG:4326",
                )  # save as geofeather
            else:
                print(
                    "NOTIFICATION: Extraction for group '{}' for area '{}' resulted in an empty df. No output will be made...".format(
                        group, area
                    )
                )
    else:
        print(
            "WARNING: No infrastructure data is found in area '{}'. Please check if OSM-file is correct and whether it intersects with polygon of area (country_shape)".format(
                area
            )
        )
...
def extract_infrastructure(local_path):
    ...
    """
    # get paths
    osm_data_path, fetched_infra_path, country_shapes_path = set_paths(
        local_path, extract_data=True
    )
    ...
    print(
        "Time to start extraction of requested assets for the following areas: {}".format(
            areas
        )
    )
    with Pool(cpu_count() - 1) as pool:
        pool.starmap(
            extract_infrastructure_per_area,
            zip(
                areas,
...
def base_calculation_per_area(area, infrastructure_systems, local_path):
    ...
    # try:
    # get paths
    grid_path, fetched_infra_path, infra_base_path, country_shapes_path = set_paths(
        local_path, base_calculation=True
    )
    ...
    print(
        "Time to import the fetched data files and put it in a dictionairy for base calculations for area: {}".format(
            area
        )
    )
    ...
    for group in groups_list:
        if (
            os.path.isfile(
                os.path.join(fetched_infra_path, "{}_{}.feather".format(area, group))
            )
            == True
        ):
            fetched_data_dict[group] = from_geofeather(
                os.path.join(fetched_infra_path, "{}_{}.feather".format(area, group))
            )  # open as geofeather

    Path(os.path.join(infra_base_path, "base_per_area")).mkdir(
    ...
    if cisi.check_dfs_empty(fetched_data_dict) == False:  # df's contain data
        ...
        if country_shape.empty == False:  # if ISO_3digit in shape_countries
            ...
            grid_data_area = (
                grid_data.loc[
                    spat_tree.query(
                        country_shape.geometry.iloc[0], predicate="intersects"
                    ).tolist()
                ]
            ).sort_index(
                ascending=True
            )  # get grids that overlap with cover_box
            grid_data_area = grid_data_area.reset_index().rename(
                columns={"index": "grid_number"}
            )  # get index as column and name column grid_number
        else:
            print(
                "Area '{}' not specified in file containing shapefiles of countries with ISO_3digit codes. Grid file will be clipped based on an overlay with infrastructure data".format(
                    area
                )
            )
            # abstract grid cells that overlap with boundaries of infrastructure data
            cover_box = gridmaker.create_cover_box(
                gridmaker.box_per_df(fetched_data_dict)
            )  # create a box based on the boundaries of the assets to be analyzed
            spat_tree = pygeos.STRtree(
                grid_data.geometry
            )  # https://pygeos.readthedocs.io/en/latest/strtree.html
            grid_data_area = (
                grid_data.loc[
                    spat_tree.query(
                        cover_box.geometry.iloc[0], predicate="intersects"
                    ).tolist()
                ]
            ).sort_index(
                ascending=True
        ...
        cisi_exposure_base_area = cisi_exposure.base_calculations(
            infrastructure_systems, fetched_data_dict, grid_data_area
        )
        ...
        for ci_system in cisi_exposure_base_area:
            if cisi_exposure_base_area[ci_system].empty == False:
                temp_df = cisi_exposure.transform_to_gpd(
                    (cisi_exposure_base_area[ci_system])
                )  # transform df to gpd with shapely geometries
                temp_df.to_file(
                    os.path.join(
                        infra_base_path,
                        "base_per_area",
                        "{}_{}.gpkg".format(area, ci_system),
                    ),
                    layer=" ",
                    driver="GPKG",
                )
                to_geofeather(
                    ...
                    os.path.join(
                        infra_base_path,
                        "base_per_area",
                        "{}_{}.feather".format(area, ci_system),
                    ),
                    crs="EPSG:4326",
                )  # save as geofeather
                ...
                cisi_exposure_base_area[ci_system].geometry = pygeos.to_wkb(
                    cisi_exposure_base_area[ci_system].geometry
                )
        print(
            "Base calculations are finished and data is exported for area: {}".format(
                area
            )
        )
        ...
        print(
            "WARNING: there is no infrastructure extracted for area '{}'. Please check if OSM-file is correct and matches polygon of area (country_shape)".format(
                area
            )
        )
        cisi_exposure_base_area = {
...
def base_calculations(local_path):
    ...
    with Pool(cpu_count() - 1) as pool:
        cisi_exposure_per_area = dict(
            pool.starmap(
                base_calculation_per_area,
                zip(
                    areas,
    ...
    # get paths
    grid_path, infra_base_path = (
        set_paths(local_path, base_calculation=True)[0],
        set_paths(local_path, base_calculation=True)[2],
    )
    ...
    cisi_exposure_base = {
        ci_system: grid_data.copy() for ci_system in infrastructure_systems
    }
    ...
    # get fetched_data_dict for area
    print(
        "Time to start summary base calcualations for the following areas: {}".format(
            areas
        )
    )
    for area in areas:
                    ...
                    if "{}".format(asset) in grid_data_area.columns:
                        for grid_cell in grid_data_area.itertuples(index=False):
                            if getattr(grid_cell, asset) != 0.0:
                                cisi_exposure_base[ci_system].loc[
                                    getattr(grid_cell, "grid_number"),
                                    "{}".format(asset),
                                ] += getattr(grid_cell, asset)
            else:
                print(
                    "WARNING: the following {}/{} combination does not exist".format(
                        area, ci_system
                    )
                )
    ...
    for ci_system in cisi_exposure_base:
        temp_df = cisi_exposure.transform_to_gpd(
            (cisi_exposure_base[ci_system])
        )  # transform df to gpd with shapely geometries
        temp_df.to_file(
            os.path.join(
                infra_base_path, "summary_basecalcs_{}.gpkg".format(ci_system)
            ),
            layer=" ",
            driver="GPKG",
        )
        to_geofeather(
            cisi_exposure_base[ci_system],
            os.path.join(
                infra_base_path, "summary_basecalcs_{}.feather".format(ci_system)
            ),
            crs="EPSG:4326",
        )  # save as geofeather
    ...
    for ci_system in infrastructure_systems:
        for area in areas:
            if (
                ...
                grid_data_area = from_geofeather(
                    os.path.join(
                        infra_base_path,
                        "base_per_area",
                        "{}_{}.feather".format(area, ci_system),
                ...
                if grid_data_area.empty == False:
                    ...
                    print(
                        "WARNING: the {}_{} file for base calculations is empty".format(
                            area, ci_system
                        )
                    )
            else:
                print(
                    "WARNING: the {}_{} file for base calculations does not exist".format(
                        area, ci_system
                    )
                )
        ...
        print(
            "Summary base calculations are done for {}. Will be exported now...".format(
                ci_system
            )
        )  # and save summary base calculations as geofather
        temp_df = cisi_exposure.transform_to_gpd(
...
def cisi_calculation(local_path, goal_area):
    ...
    # get paths
    method_max_path, method_mean_path, infra_base_path = set_paths(
        local_path, cisi_calculation=True
    )
    ...
    if "cisi_exposure_base" in globals() or "cisi_exposure_base" in locals():
        ...
        print(
            "Time to import the infrastructure datafiles containing the summary base calculations and put it in a dictionairy for CISI analysis"
        )
        cisi_exposure_base = {
            ci_system: pd.DataFrame() for ci_system in infrastructure_systems
        ...
        for ci_system in infrastructure_systems:
            if (
                os.path.isfile(
                    os.path.join(
                        infra_base_path,
                        "summary_basecalcs_{}.feather".format(ci_system),
                    )
                )
                == True
            ):
                ...
                cisi_exposure_base[ci_system] = (
                    infra_base_data  # save data in dictionary
                )
            else:
                print(
                    "WARNING: the following file summary_basecalcs_{}.feather does not exist".format(
                        ci_system
                    )
                )

    ## method 1 ##
    output_overall_max = cisi_exposure.cisi_overall_max_single(
        weight_assets,
        weight_groups,
        weight_subsystems,
        infrastructure_systems,
    ...
    to_geofeather(
        (output_overall_max[0]),
        os.path.join(method_max_path, "CISI_exposure_{}.feather".format(goal_area)),
        crs="EPSG:4326",
    )  # save as geofeather
    ...
    cisi_exposure.make_plots_automatic(
        output_overall_max[1], output_overall_max[0], goal_area, method_max_path
    )

    ## method 1 without normalization ##
    output_overall_max1 = cisi_exposure.cisi_overall_max_single_no_normalization(
        weight_assets,
        weight_groups,
        weight_subsystems,
        infrastructure_systems,
    ...
    # export as gpkg and geofeather
    to_geofeather(
        (output_overall_max1[0]),
        os.path.join(
            method_max_path_extended, "CISI_exposure_{}.feather".format(goal_area)
        ),
        crs="EPSG:4326",
    )  # save as geofeather
    ...
    # make plots of final exposure index, and sub indices, and save automatically
    cisi_exposure.make_plots_automatic(
        output_overall_max1[1],
        output_overall_max1[0],
        goal_area,
        method_max_path_extended,
    )
    ...
    if len(sys.argv) == 1:
        ...
        # receive multiple ISO-codes in a list, run specified countries
            ...
            if "]" in sys.argv[1]:
                ...
                print(
                    "FAILED: Please write list without space between list-items. Example: [NLD.LUX.BEL]"
                )
        # receive global, run all countries in the world
            ...
            glob_info = pd.read_excel(
                os.path.join(
                    "/scistor",
                    "ivm",
                    "snn490",
                    "Datasets",
                    "global_information_advanced.xlsx",
                )
            )
        ...
        # receive continent, run all countries in continent
            ...
            if len(areas) == 0:
                print(
                    "FAILED: Please write the continents as follows: Africa, Asia, Central-America, Europe, North-America,Oceania, South-America"
                )
            else:
                run_all(areas)
        else:
